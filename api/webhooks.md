...
Use the Dashboard test tools or trigger real events in sandbox mode.

## Example: Goal Contribution Handler

```python
@app.route('/webhooks/corridor', methods=['POST'])
def handle_corridor_webhook():
    event = request.json
    
    if event['type'] == 'goal.contribution':
        data = event['data']
        ...
        send_notification(
            goal_id,
            f"{contributor} contributed ${amount}!"
        )
        
        # Check if goal is complete
        if data['goal_current_amount'] >= data['goal_target_amount']:
            send_notification(goal_id, "🎉 Goal reached!")
